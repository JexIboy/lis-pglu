	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('viewArchive','retrieve','index','view','create','update','admin','download','ResolutionList','resoDetails'),
				'roles'=>array('SCR-T','SCR-RF','SCR-RC','SCR-BOK','BOKAL'),
			),
			
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('delete','download','viewArchive','retrieve','addoldreso'),
				'roles'=>array('SYSAD'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
...
//-------------------------------------------------
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Resolution']))
		{
			...
			$picture_name='';
                ...
                if($picture_file){ 
                $picture_name = $picture_file->name;
                if(!is_dir(Yii::getPathOfAlias('webroot').'/protected/document/resolution/'.substr($model->res_no,6,9).'/'.$model->reso_file)){
                	mkdir(Yii::getPathOfAlias('webroot').'/protected/document/resolution/'.substr($model->res_no,6,9).'/'.$model->reso_file);
                }

                          if(!is_dir(Yii::getPathOfAlias('webroot').'/protected/document/resolution/'.substr($model->res_no,6,9).'/'.$model->res_no)){
                             mkdir(Yii::getPathOfAlias('webroot').'/protected/document/resolution/'.substr($model->res_no,6,9).'/'.$model->reso_file);
                             $picture_file->SaveAs(Yii::getPathOfAlias('webroot').'/protected/document/resolution/'.substr($model->res_no,6,9).'/'.$model->res_no.'/'.$picture_file->getName());
                  ...
                  $model->input_by=User::model()->findByPK(Yii::app()->user->name)->emp_id;
                  //$model->reso_file=Resolution::model()->reso_file;
              $model->author=implode(',',$model->author);
			if($model->save())
				date_default_timezone_set("Asia/Manila");
    $activity=new Activity();
	...
	{
		/*$dataProvider=new CActiveDataProvider('Resolution');
